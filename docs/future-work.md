...
> V1 の4つの改善が完了した後に着手する。

### 背景

V1 のステートレス化（セクション4）により、`monas leader` はプロセスを起動直後に終了する。これは副産物として **複数の Leader を並列起動すること自体は既に技術的に実現可能** であることを意味する。

```bash
monas leader -- "機能A の実装" &
monas leader -- "機能B の実装" &  # これはすでに動く
```

Run ごとに独立した `.monas/$RUNID/` ディレクトリが生成されるため、状態管理も自然に分離される。

### 残る課題: 同一ファイルへの並列編集競合

複数の Leader（およびその配下の Member）が同じソースファイルを同時に編集する場合、コンフリクトが発生する。

**設計方針**:

tmux ベースの並列マルチエージェントシステムの実装事例を調査した結果、競合防止の最もシンプルな原則は **「ロックではなく所有権の分割」** であることが判明した。各エージェントが専用のファイルセットのみを操作することで、ロック競合を根本から排除している。

monas V2 への適用案：

- **運用ルール方式（Approach A）**: Owner が各 Leader に担当ファイル・モジュールの範囲を明示して委譲する。競合はヒューマンレイヤーで防ぐ。実装コストゼロ
- **worktree 方式（Approach B）**: 各 Leader を Git worktree の別ブランチで動かし、完了後に Owner がマージを判断する。競合は Git レイヤーで解決するがマージという新たな複雑さが入る
